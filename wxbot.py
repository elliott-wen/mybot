...
class WXBot(object):
    def __init__(self):
        ...
        self.temp_pwd  =  os.path.join(os.path.dirname(__file__),'data')
    ...
    def extract_msg_content(self, msg_type_id, msg):
        ...
        if mtype == 1:
            ...
            if self.DEBUG:
                debug_str = '    %s[Recommend]' % msg_prefix
                '    -----------------------------'
                '    | NickName: %s' % info['NickName']
                '    | Alias: %s' % info['Alias']
                '    | Local: %s %s' % (info['Province'], info['City'])
                '    | Gender: %s' % ['unknown', 'male', 'female'][info['Sex']]
                '    -----------------------------'
            ...
            if self.DEBUG:
                debug_str = '    %s[Share] %s' % (msg_prefix, app_msg_type)
                '    --------------------------'
                '    | title: %s' % msg['FileName']
                '    | desc: %s' % self.search_content('des', content, 'xml')
                '    | link: %s' % msg['Url']
                '    | from: %s' % self.search_content('appname', content, 'xml')
                '    | content: %s' % (msg.get('content')[:20] if msg.get('content') else "unknown")
                '    --------------------------'
                logging.debug(debug_str)
        elif mtype == 62:
            msg_content['type'] = 8
        ...
        while True:
            ...
            try:
                ...
                if retcode == '1100':  # 从微信客户端上登出
                    logging.error("Exit request received")
                    break
                elif retcode == '1101':  # 从其它设备上登了网页微信
                    logging.error("Other device logined")
                    break
                elif retcode == '0':
                    if selector == '2':  # 有新消息
                        r = self.sync()
                        ...
                        logging.debug('sync_check:', retcode, selector)
                        r = self.sync()
                        if r is not None:
                            self.handle_msg(r)
                else:
    ...
    def send_msg(self, name, word, isfile=False):
        ...
        if uid is not None:
            ...
            if self.DEBUG:
                logging.error('[ERROR] This user does not exist .')
            return True
    ...
    def run(self):
        ...
        # if self.restore_cookie() and self.init():
        #     logging.info("Fast forward path!")
        # else:
        if not self.get_uuid():
            logging.error("Get uuid failed")
            return
        ...
        if self.init():
            ...
            logging.error('[INFO] Web WeChat init failed')
            return
        self.status_notify()
        self.get_contact()
    ...
    def gen_qr_code(self):
        ...
        print(qr.terminal(quiet_zone=1, module_color='red', background='yellow'))
    ...
    #     with open(os.path.join(self.temp_pwd, "cookie.json"), 'wb') as f:
    #     #     self.uuid = ''
    #     # self.base_uri = ''
    #     # self.redirect_uri = ''
    #     # self.uin = ''
    ...
    #
    # def restore_cookie(self):
    #     cookie_path = os.path.join(self.temp_pwd, "cookie.json")
    #     if(os.path.exists(cookie_path)):
    #         with open(cookie_path, 'r') as f:
    #             data = json.load(f)
    #             self.uuid = data[0]
    #             self.base_uri = data[1]
    #             self.uin = data[2]
    #             self.sid = data[3]
    #             self.skey = data[4]
    #             self.pass_ticket = data[5]
    #             self.device_id = data[6]
    #             self.base_request = data[7]
    #             self.sync_key_str = data[8]
    #             self.sync_key = data[9]
    #             self.sync_host = data[10]
    #         os.unlink(cookie_path)
    #         logging.info("restoring cookie")
    #         return True
    #     else:
    #         return False
    ...
    def sync_check(self):
        ...
        try:
            ...
            logging.error("Something Wrong with the syn")
            return [-1, -1]
    ...
    def sync(self):
        ...
        try:
            ...
            logging.warning("Sync return none")
            return None
